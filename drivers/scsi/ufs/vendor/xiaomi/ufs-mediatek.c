...
static int ufs_mtk_init(struct ufs_hba *hba)
{
	...
	dev_err(dev, "xiaomi-dbug enter ufs_mtk_init.\n");
				  ...
				  struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if ((hba->dev_quirks & UFS_DEVICE_QUIRK_LIMIT_NUM_LANES_RX) &&
								of_property_read_bool(np, "ufs_limit_num_lanes_rx_1")){
			    pr_info("Number Of Active Lanes RX Has Been Limited From 2 To 1\n ");
				host_cap.rx_lanes = 1;
	}
...
static int ufs_mtk_probe(struct platform_device *pdev)
{
	...
	dev_err(dev, "xiaomi-debug enter ufs_mtk_probe.\n");
