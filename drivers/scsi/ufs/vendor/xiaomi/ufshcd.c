...
static struct ufs_dev_fix ufs_fixups[] = {
	...
	UFS_FIX(UFS_VENDOR_TOSHIBA, "THGLF2G9D8KBADG",
		UFS_DEVICE_QUIRK_PA_TACTIVATE),
	UFS_FIX(UFS_VENDOR_WDC, "SDINDDK4-64G",
		UFS_DEVICE_QUIRK_LIMIT_NUM_LANES_RX),
	END_FIX
};
...
void ufsdbg_set_err_state(struct ufs_hba *hba, char *err_reason)
{
	int offset = 9;
	...
	hba->err_stats.err_occurred++;
	while (offset) {
		memcpy(hba->err_stats.err_reason + offset, 
				hba->err_stats.err_reason + offset - 1, 32);
		offset--;
	}
	strcpy(hba->err_stats.err_reason[0], err_reason);
	dump_stack();
}

static void ufshcd_update_uic_error_cnt(struct ufs_hba *hba, u32 id, u32 val)
{
	...
	case UFS_EVT_PA_ERR:
		...
		for_each_set_bit(ec, &err_bits, UFS_EC_PA_MAX) {
			hba->uic_stats.pa_err_cnt[ec]++;
			hba->uic_stats.pa_err_cnt_total++;
		}
		break;
	case UFS_EVT_DL_ERR:
		err_bits = val & UIC_DATA_LINK_LAYER_ERROR_CODE_MASK;
		for_each_set_bit(ec, &err_bits, UFS_EC_DL_MAX) {
			hba->uic_stats.dl_err_cnt[ec]++;
			hba->uic_stats.dl_err_cnt_total++;
		}
		break;
	case UFS_EVT_DME_ERR:
		hba->uic_stats.dme_err_cnt++;
	default:
		break;
	}
}
...
ufshcd_wait_for_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	if ((ret) && (ret != -ETIMEDOUT))
		ufsdbg_set_err_state(hba, "wait_for_uic_cmd_fail\n");
#endif
	spin_lock_irqsave(hba->host->host_lock, flags);
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
	if ((err) && (err != -EAGAIN))
		ufsdbg_set_err_state(hba,"wait_for_dev_cmd_fail\n");
#endif
	return err;
}
...
int ufshcd_query_descriptor_sel_retry(struct ufs_hba *hba,
				  enum query_opcode opcode,
				  enum desc_idn idn, u8 index,
				  u8 selector,
				  u8 *desc_buf, int *buf_len)
...
EXPORT_SYMBOL_GPL(ufshcd_query_descriptor_sel_retry);
...
out:
	if (ret) {
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufsdbg_set_err_state(hba, "uic_pwr_ctrl_error\n");
#endif
...
int ufshcd_uic_hibern8_enter(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		ufsdbg_set_err_state(hba, "uic_hibern8_exit_failed\n");
#endif
			     ...
			     struct ufs_pa_layer_attr *pwr_mode)
{
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufsdbg_set_err_state(hba, "change_power_mode_fail\n");
#endif
		dev_err(hba->dev,
	...
	do {
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		if (ret)
			ufsdbg_set_err_state(hba, "link_startup_fail\n");
#endif
		...
		if (!ret && !ufshcd_is_device_present(hba)) {
			ufshcd_update_evt_hist(hba,
					       ...
					       0);
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
			ufsdbg_set_err_state(hba, "device_present_fail\n");
#endif
...
ufshcd_transfer_rsp_status(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if ((host_byte(result) == DID_ERROR) ||
		...
		ufsdbg_set_err_state(hba, "transfer_rsp_status_fail\n");
#endif
	return result;
}
...
static void ufshcd_err_handler(struct work_struct *work)
{
	...
	down(&hba->host_sem);
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
	ufsdbg_set_err_state(hba, "ufshcd_err_handler\n");
#endif
	spin_lock_irqsave(hba->host->host_lock, flags);
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufshcd_update_uic_error_cnt(hba, UFS_EVT_PA_ERR, reg);
#endif
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufshcd_update_uic_error_cnt(hba, UFS_EVT_DL_ERR, reg);
#endif
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufshcd_update_uic_error_cnt(hba, UFS_EVT_DME_ERR, reg);
#endif
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
	ufsdbg_set_err_state(hba, "ufshcd_abort\n");
#endif
...
static int ufshcd_hba_init(struct ufs_hba *hba)
{
	...
	dev_err(hba->dev, "xiaomi-debug enter ufshcd_hba_init.\n");
...
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufsdbg_set_err_state(hba, "ufshcd_link_off_fail\n");
#endif
...
out:
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufsdbg_set_err_state(hba,"ufshcd_suspend_fail\n");
#endif
...
out:
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MI_MEMORY_SYSFS)
		ufsdbg_set_err_state(hba, "ufshcd_resume_fail\n");
#endif
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	dev_err(dev, "xiaomi-debug enter ufshcd_init.\n");
...
int ufshcd_core_init(void)
{
	ufs_debugfs_init();
	return 0;
}

void ufshcd_core_exit(void)
{
	ufs_debugfs_exit();
}
