...
static int nvt_get_panel_type(struct nvt_ts_data *ts_data)
{
	...
	for (j = 0; j < 60; j++) {
		if (lockdown[1] == 0x36 || lockdown[2] == 0x36) {
			i = 0;
			break;
		}
		if (lockdown[1] == 0x44 || lockdown[2] == 0x44) {
			i = 1;
			break;
		}
		mdelay(1000);
...
static void nvt_init_touchmode_data(void)
{
	...
	xiaomi_touch_interfaces.touch_mode[Touch_Panel_Orientation][GET_DEF_VALUE] = 2;
...
void nvt_game_mode_recovery(void)
{
...
static int nvt_change_fps(void *unused)
{
	int8_t ret = 0;
	uint8_t buf[2] = {0};
	buf[0] = 0x77;
	if (!(ts->ic_state == NVT_IC_SUSPEND_IN || ts->ic_state == NVT_IC_SUSPEND_OUT)) {
		NVT_LOG("change fps enter\n");
		switch (ts->current_fps) {
		case 60:
			buf[1] = 0x00;
			break;
		case 90:
			...
			break;
		default:
			NVT_LOG("invalid fps: %d, do nothing\n");
			return -EINVAL ;
		}
		ret = nvt_touchfeature_set(buf);
		msleep(40);
		if (ret < 0) {
			NVT_ERR("change fps fail");
			return -EAGAIN;
		}
		NVT_LOG("change fps succeed");
	} else {
		NVT_LOG("skip change fps because ic is in suspend");
	}
...
static u8 nvt_panel_display_read(void)
{
	...
	if (ts->lkdown_readed) {
		...
		ret = get_lockdown_info_for_nvt(ts->lockdown_info);
		if (ret < 0) {
			NVT_ERR("can't get lockdown info %d", ret);
			return value;
		}
		value = ts->lockdown_info[1];
		ts->lkdown_readed = true;
...
static void tpdbg_suspend(struct nvt_ts_data *ts_core, bool enable)
{
	if (enable) {
		NVT_LOG("start\n");
		mutex_lock(&ts->lock);
		if (nvt_write_addr(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD, 0x12) < 0)
			NVT_ERR("tp suspend failed!");
		mdelay(20);
		mutex_unlock(&ts->lock);
		bTouchIsAwake = 0;
		NVT_LOG("end\n");
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	tp_enable_doubleclick(!!ts->db_wakeup);
	/*if true, dbclick work until next suspend*/
...
static int nvt_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata->data && ts_data) {
		...
		if (event == MI_DISP_DPMS_EARLY_EVENT) {
			...
			NVT_LOG("event=%lu, *blank=%d\n", event, *blank);
			if (ts_data->current_fps != *blank) {
				ts_data->current_fps = *blank;
				thread = kthread_run(nvt_change_fps, 0, "change_fps_thread");
				if (IS_ERR_OR_NULL(thread)) {
					NVT_LOG("failed to create kernel thread :%d", PTR_ERR(thread));
				}
				NVT_LOG("change fps notify return succeed");
			}
		}
	}
	return 0;
